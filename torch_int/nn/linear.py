...
class W8A8B8O8Linear(torch.nn.Module):
    ...
    def from_float(module: torch.nn.Linear, input_scale, output_scale):
        # assert module.weight.dtype == torch.float32, "must pass float32 input"
        # assert module.bias.dtype == torch.float32, "must pass float32 input"
        int8_module = W8A8B8O8Linear(
            module.in_features, module.out_features)
        int8_weight, weight_scale = quantize_per_tensor_absmax(module.weight.clone())
        int8_bias, bias_scale = quantize_per_tensor_absmax(module.bias.clone())
...
class W8A8BFP32OFP32LinearGELU(torch.nn.Module):
    # For fc2 and out_proj
    def __init__(self, in_features, out_features, alpha=1.0, beta=1.0):
        super().__init__()
        self.in_features = in_features
    ...
    def forward(self, x):
        ...
        x = x.view(-1, x_shape[-1])
        self.bias = self.bias.to(torch.float32)
        y = linear_gelu_a8_w8_bfp32_ofp32(
            x, self.weight, self.bias, self.a.item(), 1)
        y = y.view(*x_shape[:-1], -1)
        return y
    ...
    @staticmethod
    def from_float(module: torch.nn.Linear, input_scale):
        int8_module = W8A8BFP32OFP32LinearGELU(
            module.in_features, module.out_features)
        int8_weight, weight_scale = quantize_per_tensor_absmax(module.weight)
        alpha = input_scale * weight_scale
        int8_module.weight = int8_weight
